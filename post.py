...
vec3 local_equalizer(sampler2D texture, vec2 texCoord, vec2 texSize) {
  ...
  float midGray = midColor.r + midColor.g + midColor.b;
  for (int i = -1; i <= 1; i++) {
    for (int j = -1; j <= 1; j++) {
      
      vec2 offset = vec2(float(j), float(i));
      vec3 curColor = texture2D(texture, texCoord + onePixel*offset).rgb;
      float curGray = curColor.r + curColor.g + curColor.b;
      if (curGray <= midGray) {
        cfd = cfd + vec3(1./9.);
      }
